...
namespace DAQRI
{
	...
	public class TrackedObjectEditor : SceneSingletonEditor
	{
		...
		private static bool BrowseAndLoadTexture(TrackedObject to)
		{
			...
			if (externalPath.Length != 0)
			{
				...
				if (to.name.StartsWith("GameObject") || to.name.StartsWith("Tracked Object") || to.name.StartsWith("TO_", StringComparison.Ordinal)) 
				{
					to.name = "TO_" + filename.Remove(filename.Length - 4);
				}
				to.targetName = filename;
		...
		private static void UpdateMaterial (TrackedObject to)
		{
			...
			to.gameObject.AddMissingComponent<MeshRenderer>().sharedMaterial = material2;
		}
